...
void afficherValeurs(float *valeurs,unsigned long nbrValeurs){
    for(unsigned int i=0;i<nbrValeurs;i+=10){
        printf("valeur %d = %f\n",i,valeurs[i]);
    }
}
...
void changementBase(float **valeurs,unsigned long nbrValeurs){
    ...
    for(unsigned long i=0;i<nbrValeurs;i++){
        temp=(*valeurs)[i];
        if(temp>valMax){
            valMax=temp;
            indiceMax=i;
...
int ecritureDescripteur(char *nomFichierDescripteur,descripteur des){
  ...
  for(int i=0;i<nbr_parametres_ecriture_audio;i++){
    switch(i){
      // ecriture de l'ID
      ...
      case 3 : 
        for(unsigned int k=0;k<des.valeurs.nbLig;k++){
            for(unsigned int j=0;j<des.valeurs.nbCol;j++){
