...
int creerMatrice(unsigned int nbLig, unsigned int nbCol, matrice *mat){
  ...
  mat->nbLig=nbLig;
  mat->nbCol=nbCol;
  unsigned long int **dimension=(unsigned long int **)malloc(sizeof(unsigned long int *)*nbLig);
...
matrice initialiserMatrice(matrice mat){
  for(unsigned int i=0;i<mat.nbLig;i++){
    for(unsigned int j=0;j<mat.nbCol;j++){
